...
class _CameraDetectorScreenState extends State<CameraDetectorScreen> {
  ...
  Future<void> _openCamera() async {
    ...
    try {
      ...
      if (!mounted) return;
      // Small delay to let the camera fully release resources
      ...
      if (!mounted) return;
      // Permission denied, camera not available, or flash-related crash
  ...
  Widget _buildSuccessState() {
    final bool isUnrecognized = _predictionResult!['is_unrecognized'] ?? false;
      ...
      return Column(
        children: [
          Container(
            ...
            child: Icon(Icons.help_outline_rounded, color: AppColors.textSecondary, size: 32),
          ),
          const SizedBox(height: 16),
          Text('Unrecognized Object', style: AppTextStyles.labelLarge),
          const SizedBox(height: 8),
          Text(
            'The photo doesn\'t look like a known mushroom species with high confidence. Please try a clearer, closer shot.',
            style: AppTextStyles.bodySmall,
            textAlign: TextAlign.center,
          ),
        ],
    ...
    final isPoisonous = _predictionResult!['is_poisonous'] ?? false;

    // Determine color based on classification
    Color classColor;
    IconData classIcon;
    final lowerClass = mainClass.toString().toLowerCase();
    if (lowerClass.contains('poisnous')) {
      classColor = AppColors.danger;
      classIcon = Icons.warning_rounded;
    } else if (lowerClass == 'non_poisnous_edible') {
      classColor = AppColors.success;
      classIcon = Icons.check_circle_rounded;
    } else if (lowerClass == 'non_poisnous_non_edible') {
      classColor = AppColors.warning;
      classIcon = Icons.info_rounded;
    } else {
      classColor = AppColors.info;
      classIcon = Icons.help_rounded;
    }
    ...
    return Column(
      children: [
        ...
        // More Details Button
        if (_detectedSpecies != null && !isUnrecognized)
          SizedBox(
            width: double.infinity,
